...
**Advancing.** Press `→` or click anywhere outside a control → next image with a ~400ms cross-fade. Press `←` to go back through the session's forward breadcrumbs (LIFO). Back is bounded by the session — it stops at the very first slide the user saw on the page. When there's nothing to go back to (history empty), the prev button is rendered in a visually disabled state (`opacity: 0.3`, `cursor: not-allowed`, no hover/active effect) and the browser blocks the click. On touch-only devices, a *tap* on the image does NOT advance — only a horizontal swipe (left = next, right = prev) or the nav buttons navigate. The tap-to-advance gesture is desktop / pointer-device only; on touch, accidental taps are too easy. Order is shuffled per visit. When the user reaches the end of the batch, the list re-shuffles and continues from the start (infinite loop). **Rapid clicks while an image is still loading are ignored** — the cursor (and history) only advance once a slide has actually displayed, so a click-back after spamming next reliably returns to the previously-shown image.
...
When playing, the viewer auto-advances at the chosen interval. Clicking the **Speed** button cycles through `8s → 16s → 32s → 64s` and wraps; the button's label always shows the current setting. Changing the speed while playing restarts the timer with the new value so it applies immediately. Any manual input (click, `→`, `←`, swipe, ‹, ›) also resets the timer but does NOT pause the slideshow. The setting is per-session (not persisted). Default: 8 s. Keyboard shortcuts: `→` next, `←` prev, `Space` or `p` toggles play/pause, `i` toggles the info sidebar, `f` toggles fullscreen, `esc` closes the sidebar (and exits fullscreen via the browser's built-in behavior).
